...
PMI <- function(.tbl,geo) {.tbl %>%
    ...
    select(-8,-12) %>%
    rename("Taux déquipement en accueil collectif<sup>1</sup>" = 
    ...
    mutate(`Taux déquipement en accueil collectif<sup>1</sup>`=
             round(`Taux déquipement en accueil collectif<sup>1</sup>`, 1),
           `Taux de scolarisation des enfants de 2 ans<sup>2</sup>`=
...
hPMI <- function(){
  ...
  tab %>%
    arrange(desc(`Taux déquipement en accueil collectif<sup>1</sup>`)) %>%
    hchart("bar",
           hcaes(x=Nom,y=`Taux déquipement en accueil collectif<sup>1</sup>`,
                 color=ifelse(str_detect(Nom,"Bourgogne"),"#6a5acd","#ffd700")),
...
tab <- tab %>%  
  select(Nom,
         ...
         `Taux déquipement en accueil collectif<sup>1</sup>`,
         `Taux de scolarisation` = 
         `Taux de scolarisation des enfants de 2 ans<sup>2</sup>`) %>%
