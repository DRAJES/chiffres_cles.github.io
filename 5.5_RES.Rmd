...
res_infra <- function(geo) {
  ...
  tab_nombre <- 
    ...
    credits_res() %>%
    footnote(number = c("structurant : salles de sport collectifs \n",
                        "proximité : terrains en accès libre"), 
             footnote_as_chunk = T) %>%
  ...
  return(list(tab_nombre=tab_nombre,tab_taux=tab_taux))
}
...
popup <- carte@data %>% select(Libellé, Population = pop,
                               `Nombre d'équipements`= équipements,`Taux d'équipements`=txequip,
                               `Taux de bassins`=txbassins, `Taux de grands jeux`=txgdsjeux,
                               `Taux de structurant`=txstructurant,`Taux de gymnases`=txgymnases,
                               `Taux de tennis`=txtennis,`Taux de tennis couverts`=txtennis_couv,
...
.map %>%
  ...
  addMinicharts(centroid(carte)[,1],centroid(carte)[,2] ,
                       ...
                       chartdata = carte@data %>% 
                                        mutate(`tennis extérieurs`=tennis-`dont couverts`,
                                               autres = équipements - structurant - tennis -
                                                 `grands jeux` - proximité - `tous bassins`) %>%
                                        select(structurant,`grands jeux`,
                                               `tennis extérieurs`, `tennis couverts`=`dont couverts`,
                                               `terrains de proximité`=proximité, `tous bassins`, autres),
                colorPalette = coul,
...
tab %>%
  mutate(`taux d'équipements`=color_bar("orange")
...
```{r}
res_infra(geo=EPCI)[[1]]
```

### Taux d'équipements

```{r}
res_infra(geo=EPCI)[[2]]
...
```{r}
res_infra(geo=BV2012)[[1]]
```

### Taux d'équipements 

```{r}
res_infra(geo=BV2012)[[2]]
```
