...
.map %>%
  addPolygons(color = "#2F4F4F",  weight = 2, opacity = 0.6,
   ...
   addPolygons(color = "#2F4F4F", weight = 2, opacity = 0.6, 
               ...
               popup = popupTable(carte@data,
                                 feature.id=F,row.numbers = F),
              group = "taux structurant") %>%
    addLegend( pal = palstr, values = ~`taux structurant`,   
               "bottomright",title = "taux d'équipements structurant  <br> pour 10 000 habitants",
               group = "taux structurant") %>%
  ...
  addMinicharts(centroid(carte)[,1],centroid(carte)[,2] ,
                       #maxValues = 100,
                       type = "pie",
                       chartdata = carte@data %>% select("structurant":"tous bassins"),
                       colorPalette = coul,
    ...
    syncWith("minicharts") %>%
    addLayersControl( overlayGroups = c("taux équipements",
                                     "taux structurant",
                                     "taux grands jeux",
                                     "taux proximité", 
                                     "taux bassins"),
...
tab %>% 
  taux_res() %>%
  select(1:3,19:26) %>%
  select(1:3,`Taux d'équipements`=4,structurant=txstructurant,
         `dont gymnases`=txgymnases,tennis=txtennis,`dont couverts`=txtennis_couv,
         `grands jeux`=txgdsjeux,proximité=txproxi,`tous bassins`=txbassins) %>%
  arrange(desc(équipements)) %>%
