...
coul <- c("#4876FF", "#008B00", "#32CD32","#CD69C9", "#CD96CD", "#FFB5C5" , "#00BFFF","#FF7F50","#ADFF2F","#C0C0C0","#DCDCDC")
 ...
 taux_res <-  function(.tbl)  {
   .tbl %>% mutate(txequi=round(10000*equ/pop,1),
 ...
 carto_res <- function(geo=REG) {
  ...
  summarise(equ=n(),
            ...
            tennis_couv=sum(EquipementCateg == "Courts de tennis" & 
                              NatureLibelle %in% c('Extérieur couvert','Découvrable', 'Intérieur'),
                            na.rm = T ),
            tennis_ext=sum(EquipementCateg == "Courts de tennis" & 
                             !NatureLibelle %in% c('Extérieur couvert','Découvrable', 'Intérieur'),
                           na.rm = T ),
...
leafRES <- function(x=1) {popup_res <- res@data %>% 
                  select(LIBGEO,
                         Population=pop.y,
                         `nb équipements`=equ,
                         `Tx équipements`=txequi,
                         `Tx bassins`=txbassins,
                         `Tx grands jeux`=txgdsjeux,
                         `Tx structurant`=txstructurant,
                         `Tx tennis`=txtennis,
                         `Tx proximité`=txproxi)

leaflet(res) %>%   
  addPolygons(color = "#2F4F4F",  weight = 2, opacity = 0.6,
              fillColor = ~pal(txequi),  fillOpacity = 0.6,
              fill = TRUE ,smoothFactor = 2,
              popup = popupTable(popup_res,
                                 feature.id=F,row.numbers = F),
              group = "taux équi/10 000 hab") %>%
   addPolygons(color = "#2F4F4F", weight = 2, opacity = 0.6, 
               fillColor = ~pal(50000*structurant/pop.y), fillOpacity = 0.6,
               fill = TRUE, smoothFactor = 2,
               popup = popupTable(popup_res,
                                 feature.id=F,row.numbers = F),
              group = "taux structurant/50 000hab") %>%
   addPolygons(color = "#2F4F4F", weight = 2, opacity = 0.6, 
               fillColor = ~pal(50000*gdsjeux/pop.y), fillOpacity = 0.6,
               fill = TRUE, smoothFactor = 2,
               popup = popupTable(popup_res,
                                 feature.id=F,row.numbers = F),
              group = "taux grands jeux/50 000hab") %>%
    addPolygons(color = "#2F4F4F", weight = 2, opacity = 0.6, 
                fillColor = ~pal(50000*proxi/pop.y), fillOpacity = 0.6, 
                fill = TRUE,  smoothFactor = 2,
                popup = popupTable(popup_res,
                                 feature.id=F,row.numbers = F),
              group = "taux proxi/50 000hab") %>%
   addPolygons(color = "#2F4F4F", weight = 2, opacity = 0.6, 
               fillColor = ~pal(400000*bassins/pop.y), fillOpacity = 0.6, 
               fill = TRUE,  smoothFactor = 2,
               popup = popupTable(popup_res,
                                 feature.id=F,row.numbers = F),
  ...
  addMinicharts(centroid(res)[,1],centroid(res)[,2] ,
                       #maxValues = 100,
                       type = "pie",
                       chartdata = res@data %>% select("bassins":"autres"),
                       colorPalette = coul,
                       width = sqrt(x*res$equ) ,  #paramètre d'échelle des charts
                       opacity = 0.6) %>% syncWith("minicharts") %>%
    contour_bfc() %>%
  addLayersControl( baseGroups = c("taux équi/10 000 hab","taux structurant/50 000hab",
                                   "taux grands jeux/50 000hab","taux proxi/50 000hab", 
                                   "taux bassins/400 000hab"),
                    overlayGroups = c("minicharts"),
                    options = layersControlOptions(collapsed = F,autoZIndex = T)) %>%  
  addLegend( pal = pal, values = ~txequi,   
            "bottomright",title = "taux d'équipements") }
...
equip %>% 
  ...
  select(REG,région=LIBGEO,équipements=equ,
         ...
         `tennis`=tennis, `dont couverts`=tennis_couv,
         `grands jeux`=gdsjeux, proximité=proxi,`tous bassins`= bassins, 
         `taux d'équipements`= txequi) %>%
...
### Taux d'équipements par habitants et répartition
...
hchart( equip %>% 
          ...
          left_join(.,appartenance %>% 
                    ...
                    by=c("REG"="CODGEO") ) %>%
          select(region, `taux bassins`:autres) %>%
          pivot_longer(-1,'type')  ,
...
equip %>% 
  filter(REG=='27') %>% 
  group_by(`Famille d'équipement`=EquipementFamille,`Type d'équipement`=EquipementTypeLib) %>%
  summarise(`nombre d'équipements`=n(),
            couverts= sum(NatureLibelle %in% c('Extérieur couvert','Découvrable','Intérieur'),
                          na.rm = T ),
...
res <- sp::merge(regwgs,res,by="REG")
...
equip %>% 
  ...
  select(DEP,département=LIBGEO,équipements=equ,
         ...
         `tous bassins`= bassins, `taux d'équipements`= txequi) %>%
...
hchart( equip %>% 
          ...
          arrange(desc(txequi)) ,
  "bar",hcaes(x=reorder(département,txequi),y=txequi,
...
hchart( equip %>% 
          ...
          left_join(.,appartenance %>% 
                      ...
                      select(CODGEO,département=LIBGEO),
                    by=c("DEP"="CODGEO") ) %>%
          select(département,`taux bassins`:autres) %>%
          pivot_longer(-1,'type')  ,
...
res <- carto_res(DEP) %>%
  left_join(.,appartenance %>% 
              filter(NIVGEO=="DEP") %>% 
              select(CODGEO,departement=LIBGEO),
            by=c("DEP"="CODGEO") ) 

res <- sp::merge(depwgs,res,by="DEP")
...
equip %>% 
  ...
  left_join(.,appartenance %>% 
            ...
            by=c("EPCI"="CODGEO") ) %>%
  select(EPCI,`nom EPCI`=LIBGEO,équipements=equ,
         `taux d'équipements`=txequi,structurant=txstructurant,`dont gymnases`=txgymnases,
         tennis=txtennis,`dont couverts`=txtennis_couv,`grands jeux`=txgdsjeux,`proximité`=txproxi,
...
res <- carto_res(EPCI) %>%
  left_join(.,appartenance %>% 
              filter(NIVGEO=="EPCI") %>% 
              select(CODGEO,EPCI=LIBGEO),
            by=c("EPCI"="CODGEO") ) 

res <- sp::merge(epcicarto,res,by="EPCI")
  
leafRES(x=10) %>% contour_depbfc()
```
...
equip %>% 
  ...
  select(BV2012,`bassin de vie`=LIBGEO,équipements=equ,
         structurant,`dont gymnases`=gymnases,`tennis`=tennis, `dont couverts`=tennis_couv,
         `grands jeux`=gdsjeux, proximité=proxi,`tous bassins`= bassins) %>%
  arrange(desc(équipements)) %>%
  
  kable("html", format.args = list(decimal.mark = ",", big.mark = " "),
       align="rcrrrrrrrrr", escape = F) %>% 
...
```{r}
res <- carto_res(BV2012) %>%
  left_join(.,appartenance %>% 
              filter(NIVGEO=="BV2012") %>% 
              select(CODGEO,BV2012=LIBGEO),
            by=c("BV2012"="CODGEO") ) 
