...
load("data/jeunesse/ACM_pano.RData")
...
**`r prettyNum(round(ACM_SH[6,3]/100,0)*100,big.mark = ",")`** lieux d'accueils sans hébergement en BFC\
...
**`r prettyNum(round(ACM_colo[2,3]/100,0)*100,big.mark = ",")`** séjours avec hébergement en BFC
...
```{r}
ACMcarte <- ACM_colo_dep %>% slice(10) %>% pivot_longer(-1) %>% bind_cols(DEP=c('BFC','21','25','39','58','70','71','89','90','FR'))
ACMcarte <- sp::merge(depwgs,ACMcarte,by="DEP")
...
leaflet(ACMcarte) %>% 
  ...
  addCircles(centroid(ACMcarte)[,1],centroid(ACMcarte)[,2],
             ...
             popup = popupTable(ACMcarte@data %>% 
                                   select(Département=DEP,`Nombre de journées enfants`=value) ,
...
**`r prettyNum(round(ACM_scout[5,3]/100,0)*100,big.mark = ",")`** places de scoutisme en Bourgogne-Franche-Comté
