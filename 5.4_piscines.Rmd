...
load("C:/Users/plebre/Documents/projets R/DRAJES/data/sport/piscines.RData")
load("C:/Users/plebre/Documents/projets R/DRAJES/data/sport/bassins.RData")
load("C:/Users/plebre/Documents/projets R/DRAJES/data/BPE/ecoles.RData")

load("C:/Users/plebre/Documents/projets R/DRAJES/data/sport/iso.RData")
...
La région Bourgogne-Franche-Comté compte **283** piscines publiques accessibles aux scolaires en 2020 pour une surface de nage par habitant de **190 m²**.  
**124** d'entre elles sont couvertes.
...
piscines %>% group_by(REG) %>% summarise_if(is.numeric,sum) %>%
  ...
  dplyr::select (région=REG, LIBGEO, `Nombre de piscines publiques`=nbpiscines,`Piscines couvertes`=nbpiscines_couv,`Piscines saisonnières`=nbpisc_saison,
...
L'éloignement joue un rôle déterminant dans surface de baignade. Le Jura, département montagneux, la Saône et Loire et l'Yonne, départements peu denses, sont bien dotées. à l'Inverse, les départements plus denses ou ceux où la population se concentre dans les grandes villes sont moins dotés. Le territoire de Belfort est ainsi le moins bien équipé sur la surface natatoire pour 10 000 habitants.
...
piscines %>% group_by(DEP) %>% filter(DEP %in% depbfc) %>% summarise_if(is.numeric,sum) %>%
  ...
  select (Département=DEP, LIBGEO, `Nombre de piscines publiques`=nbpiscines,`Piscines couvertes`=nbpiscines_couv,`Piscines saisonnières`=nbpisc_saison,
          Population=pop,`surface de baignade pour 10 000 hab`=surfnat,tranches=surfnat_rec) %>%
 kable("html", format.args = list(decimal.mark = ",", big.mark = " "),
      align="rcrrrrlrr",
      escape = F) %>% 
...
piscines %>% filter(REG=='27')  %>% group_by(EPCI) %>% summarise_if(is.numeric,sum) %>%
  ...
  select (EPCI, LIBGEO, `Nombre de piscines publiques`=nbpiscines,`Piscines couvertes`=nbpiscines_couv,`Piscines saisonnières`=nbpisc_saison,
          Population=pop,`surface de baignade pour 10 000 hab`=surfnat,tranches=surfnat_rec) %>%
 kable("html", format.args = list(decimal.mark = ",", big.mark = " "),
      align="rcrrrrlrr",
      escape = F) %>% 
...
Les bassins de vie égalisent un peu le poids des grandes agglomération en mettant les piscines en rapport à la population. Toutefois, certaines sortent du lot comme Vesoul qui est très bien doté, et les bassins de vie des montagne du Jura qui propose des infrastructures nombreuses (Champagnole, Saint-Claude).  
...
piscines %>% filter(REG=='27')  %>% group_by(BV2012) %>% summarise_if(is.numeric,sum) %>%
  ...
  select (BV2012, LIBGEO, `Nombre de piscines publiques`=nbpiscines,`Piscines couvertes`=nbpiscines_couv,`Piscines saisonnières`=nbpisc_saison,
          Population=pop,`surface de baignade pour 10 000 hab`=surfnat,tranches=surfnat_rec) %>%
 kable("html", format.args = list(decimal.mark = ",", big.mark = " "),
      align="rcrrrrlrr",
      escape = F) %>% 
...
Row {data-height="800"}
-----------------------------------------------------------------------
### Temps d'accès aux piscines de la région
