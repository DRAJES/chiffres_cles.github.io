...
leaflet(piscinesdep) %>%
  ...
  addPolygons(weight=2,opacity = 0.6,color = "#2F4F4F", fill=T, 
                           fillColor = ~pal(tranche), fillOpacity = 0.8, smoothFactor = 2 ) %>%
  ...
  addCircles(centroid(piscinesdep)[,1],centroid(piscinesdep)[,2],
             ...
             popup = popupTable(piscinesdep@data %>% 
                                   select(Département=LIBGEO,pop.x,nbpiscines,nbpiscines_couv,nbpisc_saison,surfnat,tranche),
...
leaflet(piscinesepci) %>%
  ...
  addPolygons(weight=2,opacity = 0.6,color = "#2F4F4F", fill=T, 
              ...
              popup = popupTable(piscinesepci@data %>% 
                                   select(EPCI=NOM,pop,nbpiscines,nbpiscines_couv,nbpisc_saison,surfnat,tranche),
                                 feature.id=F,row.numbers = F)) %>%
  addPolygons(data=reg27carto,group  = "BFC",fill = F,color="#9370DB",weight = 5,opacity = 1, smoothFactor = 5) %>%
...
leaflet() %>% 
  ...
  addPolygons(data=reg27carto,fill=F,weight = 4,color = "white",group = "écoles", smoothFactor = 5 ) %>%
  addPolygons(data=dep27carto,fill=F, weight = 3,color = "white",group = "écoles", smoothFactor = 2) %>%
