...
pal <- colorBin("RdYlBu", domain =-5:5 ,
                bins=c(-Inf, -1, -0.5,0,0.5, 1 , Inf), 
                reverse = T)
...
```{r,echo=F}
carte <- regwgs
carte@data <- carte@data %>% select(Région=REG,pop,evol)
...
```{r,echo=F}
carte <- depwgs
carte@data <- carte@data %>% select(Département=DEP,pop,evol)
...
```{r,echo=F}
carte <- epcicarto
carte@data <- carte@data %>% select(EPCI,pop,evol)
...
```{r,echo=F}
carte <- bvcarto
carte@data <- carte@data %>% select(BV2012,pop,evol)
