...
```{r,echo=F}
carte <- regwgs |> 
  dplyr::select(Région=REG,geometry,pop,evol)  |>  
  left_join(tab ,
            by=join_by(Région))
...
```{r,echo=F}
carte <- depwgs |> 
  dplyr::select(Département=DEP,geometry,pop,evol)

carte <- carte |> 
  left_join(dep_tab %>% 
                 select(-Région) %>% ungroup() |> 
                 demo(geo=Département),
               by="Département") |> 
  left_join(tab |> select(Département,evol68_20),
            by="Département")
...
```{r,echo=F}
carte <- epcicarto |> 
  dplyr::select(EPCI=CODE_SIREN,geometry,pop,evol)  |>  
  left_join(tab,
            by=join_by(EPCI))
...
```{r,echo=F}
carte <- bvcarto |> 
  dplyr::select(BV2022,geometry,pop,evol)  |>  
  left_join(tab,
            by=join_by(BV2022))
