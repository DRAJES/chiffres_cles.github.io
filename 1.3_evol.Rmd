  ...
  addCircles(centroid(regwgs)[,1],centroid(regwgs)[,2],weight=4,
             ...
             popup = popupTable(regwgs@data %>%
                                  ...
                                  left_join(.,region_tab %>%
                                              select(Région,Nom,Population,`15-29 ans`,`65 ans et +`,
                                                     `évolution annuelle`, `evo15/29`,`evo>65`)
                                            ,by="Région"),
                                feature.id=F,row.numbers = F) )%>%
    addLegend(pal = pal3, values =  regwgs$evol, opacity = 0.7, 
              title = "évolution annuelle", position = "bottomright",labFormat = labelFormat(suffix =  " %"))
```
...
Seuls deux départements gagnent de la population entre 2013 et 2019 : la Côte d'Or et le Doubs, portés par l'attractivité de leurs capitales Dijon et Besançon.La Nièvre est le département où la population décroît le plus en 5 ans.
             ...
             popup = popupTable(depwgs@data %>%
                                  ...
                                  left_join(.,dep_tab %>%
                                              select(Région,Département,Nom,Population,`15-29 ans`,
                                                     `65 ans et +`,`évolution annuelle`, `evo15/29`,`evo>65`)
                                            ,by="Département"),
                                feature.id=F,row.numbers = F) )%>%
    addLegend(pal = pal3, values =  regwgs$evol, opacity = 0.7, 
              title = "évolution annuelle", position = "bottomright",labFormat = labelFormat(suffix =  " %"))
  ...
  addCircles(centroid(epcicarto)[,1],centroid(epcicarto)[,2],
             ...
             popup = popupTable(epcicarto@data %>% 
                                  ...
                                  left_join(.,epci27_tab %>%
                                              select(EPCI,Nom,Population,`15-29 ans`,`65 ans et +`,
                                                     `évolution annuelle`, `evo15/29`,`evo>65`)
                                            ,by="EPCI"),
                                feature.id=F,row.numbers = F) )%>%
    addLegend(pal = pal3, values =  regwgs$evol, opacity = 0.7, 
...
leaflet(bvcarto) %>%  
  ...
  addCircles(centroid(bvcarto)[,1],centroid(bvcarto)[,2],
             ...
             popup = popupTable(bvcarto@data %>% 
                                  select(BV2012) %>%
                                  left_join(.,bv27_tab %>%
                                              select(BV2012,Nom,Population,`15-29 ans`,`65 ans et +`,
                                                     `évolution annuelle`, `evo15/29`,`evo>65`)
                                            ,by="BV2012"),
                                feature.id=F,row.numbers = F) ) %>%
  addLegend(pal = pal3, values =  regwgs$evol, opacity = 0.7, 
