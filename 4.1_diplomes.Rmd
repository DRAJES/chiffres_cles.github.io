...
```{r}
bafaregm %>% filter (année %in% 2019:2021 & sexe =='Ensemble') %>%
  bind_cols (bafdregm %>% filter (année %in% 2019:2021 & sexe =='Ensemble') %>% pull(`BAFD`)) %>%
...
bind_cols(  bafdregm %>% filter(sexe=="Ensemble" & année %in% c(2011,2021)) %>% 
  ...
  select(CODGEO=CODGEO...2,region,BAFA19=BAFA_2019,BAFD19,BAFA20=BAFA_2020,BAFD20,
         BAFA21=BAFA_2021,BAFD21,`age BAFA`, `age BAFD`,`femmes BAFA`,`femmes BAFD`, `BAFA %`, `BAFD %`,Ensemble=`évo`,Femmes=`évo Femmes`, Hommes=`évo Hommes`) %>%
  
    
 kable(format.args = list(decimal.mark = ",", big.mark = " "), escape = F,
    align="clrrrrrrllllllccc")%>%
  kable_styling("hover", full_width = F) %>%
  add_header_above(c( " "=2,"2019"=2,"2020"=2, "2021"=2, "âge médian 2021 (années)"=2,"Taux de femmes 2021 (%)"=2,"évolution 2011-2021 (%)"=2,"Effectifs annuels"=3) ) %>%
  row_spec(15, bold=T) %>%
...
```{r}
  ...
  type= "areaspline", stacking = "normal",
...
bafadepm %>% filter(dep %in% depbfc) %>%
  ...
  mutate (evo=100*(`2021`-`2011`)/`2011` ) %>% pull (evo) ) %>%
   ...
   bind_cols( bafadepm %>%filter(dep %in% depbfc & sexe=="Hommes")  %>% # sparkine
                group_by(dep) %>%
                summarise(`évo Hommes`=spk_chr(BAFA)) ) %>%
  select(dep=dep...2,département=deplib,`2019`,`2020`,`2021`,`age médian (ans)`=`age BAFA`, `Taux de femmes (%)`=`femmes BAFA`,`évolution 2011-2021 (%)`=`evo BAFA`, Ensemble=`évo`,Femmes=`évo Femmes`, Hommes=`évo Hommes`) %>%
