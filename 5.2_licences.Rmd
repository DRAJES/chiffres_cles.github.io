...
```{r}
tab <- 
  lic_reg %>% rename(codefed=1) %>%
  mutate(type_fede = 
           case_when(codefed > '100' & codefed < '200' ~ "unisport olympique",
                     codefed > '200' & codefed < '300' ~ "unisport non olympique",
                     codefed > '400' & codefed < '700' ~ "multisport",
                     TRUE ~ "autres") ) %>%
...
hw_grid(
  tab  %>% 
   ...
   tab  %>% 
    select(codefed=1,fede=2,FR="...22",type_fede) %>%
    mutate(txFrance=round(100*FR/sum(FR,na.rm=T) ,1) ) %>% 
    arrange(desc(txFrance))  %>%
    
    data_to_hierarchical(c(type_fede,fede),txFrance) %>%
    
    hchart(type="treemap") %>%
    hc_subtitle(text="<b>France</b>") %>%
    hc_add_theme(thm) ,
