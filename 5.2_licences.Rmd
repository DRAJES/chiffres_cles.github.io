...
licreg <- licreg %>% rownames_to_column(var="REG") %>%
                 left_join(.,regwgs@data %>% dplyr::select(REG=INSEE_REG,pop)
                           ,by="REG" )    %>%
                  adorn_totals("row",name = "FR") %>%
                  mutate(txlic=round(100*V1/pop,1))
licreg[14,3] <- "France"

hchart(licreg %>% slice(-14) %>% arrange(desc(V1)),"bar",
       hcaes(x=reorder(region,V1),y=V1,color=ifelse(REG=="27","#6a5acd","#ffd700") ),
...
hchart(licreg %>% arrange(desc(txlic)),"bar",
       hcaes(x=reorder(region,txlic),y=txlic,
...
addLegend( position = "bottomright",pal = pal2 ,title = "taux de pénétration sportive",
           values = 100*(cartosportreg$licences_tot/cartosportreg$pop) )
```
...
lic_reg %>%  dplyr::select(2,"11":"94",France="...22") %>%
  ...
  footnote(general = " MEDES recensement des licences sportives et des clubs 2021", general_title = "Source : ",
...
hchart ( lic_reg %>%  dplyr::select(2,"11":"94"
    ...
    pivot_longer(-`Codes régions`, 'REG') %>% 
      ...
      left_join(.,appartenance %>% filter(NIVGEO=="REG") %>%
                  select(CODGEO,region=LIBGEO),by=c("REG"="CODGEO") ) , 
...
```{r}
  hchart(fede_sexe_dep %>% 
           ...
           arrange(desc(txfem)) %>%
           left_join(lic_dep %>% 
                       select(fede=1,"Fédération"=2),by="fede"),
...
licdep <- licdep %>%
            ...
            left_join(.,depwgs@data %>% 
                        dplyr::select(dep=INSEE_DEP,pop),by="dep" ) %>%
            adorn_totals("row",name = "FR") %>%
            mutate(txlic=round(100*V1/pop,1)) 
...
hchart(licdep %>% 
  ...
  "bar", hcaes(x=reorder(departement,V1),y=V1,
        ...
        showInLegend = T, name = "Licences", pointWidth = 1) %>%
  hc_add_series(licdep %>% 
                  filter(dep %in% depbfc) %>% 
                  arrange(desc(V1)),
           "bar", hcaes(x=reorder(departement,V1),y=V1),
                  color="#6a5acd", 
                  showInLegend = F, name = "Licences", pointWidth = 3) %>%
...
hchart(licdep %>% 
  ...
  "bar", hcaes(x=reorder(departement,txlic),y=txlic,
         ...
         showInLegend = T, name = "Taux de licences", pointWidth =  1 ) %>%
  hc_add_series(licdep %>% 
                  filter(dep %in% depbfc | dep == "FR") %>%
                  arrange(desc(txlic)),
          "bar",  hcaes(x=reorder(departement,txlic),y=txlic,
                        color=ifelse(dep %in% depbfc,"#6a5acd","#ff7f50") ),
                  showInLegend = F, name = "Taux de licences (%)", pointWidth = 3) %>%
  hc_legend(title=list(text=" France/BFC (Clic pour zoomer) " )) %>%
  hc_xAxis(title=list(text="Départements")) %>%
  hc_yAxis(title=list(text="Taux de licences")) %>%
  hc_add_theme(thm) 
...
leaflet(cartosportdep) %>% 
  ...
  addCircles(centroid(cartosportdep)[,1],centroid(cartosportdep)[,2],
             ...
             popup = popupTable(cartosportdep@data %>% 
                                ...
                                feature.id=F,row.numbers = F)) %>%
addLegend( position = "bottomright",pal = pal2 ,title = "taux de pénétration sportive",
           values = 100*(cartosportdep$licences_tot/cartosportdep$pop) )
```
...
lic27epci %>%
  select(EPCI,LIBGEO,licences,txlic,txlicf,txlich,txlicjeunes,txfemmes,txjeunes) %>% 
  ...
  footnote(general = " MEDES recensement des licences sportives et des clubs 2019", general_title = "Source : ",
           number = c("F= Fédération \n","FF = Fédération Française"), footnote_as_chunk = T) %>%
  ...
  addCircles(centroid(lic_epci)[,1],centroid(lic_epci)[,2],weight=4,
             ...
             popup = popupTable(lic_epci@data %>% 
                                  ...
                                  mutate(txlicjeunes= color_tile('white', 'lightgreen', alpha=0.5)
                                        (digits(txlicjeunes,1,decimal.mark=","))),
                                feature.id=F,row.numbers = F) ) %>%
  addLegend( position = "bottomright",pal = pal2 ,values = ~txlic,
