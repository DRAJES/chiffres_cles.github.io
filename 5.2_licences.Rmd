...
leaflet(cartosportreg) %>% addProviderTiles(providers$CartoDB.Positron) %>% 
  ...
  addPolygons(weight=2,opacity = 0.6,color = "#2F4F4F", fill=F, smoothFactor = 2 ) %>%
...
lic_reg[39,1] <- "total"
fedsexe[39,1] <- "total"
lic_reg %>%  dplyr::select(1:2,"11":"94",`France métro`="...22") %>% rename("Code fédération"="...1","Fédération"="Codes régions") %>%
    slice(2:38) %>% arrange (desc(`France métro`)) %>% 
  bind_rows(lic_reg %>% dplyr::select(1:2,"11":"94",`France métro`="...22") %>% rename("Code fédération"="...1","Fédération"="Codes régions") %>% slice(39) ) %>%
...
fedsexe[91,1] <- "total"
lic_reg %>%  dplyr::select(1:2,"11":"94",`France métro`="...22") %>% rename("Code fédération"="...1","Fédération"="Codes régions") %>%
    slice(41:90)  %>% arrange (desc(`France métro`)) %>% 
  bind_rows(lic_reg %>% dplyr::select(1:2,"11":"94",`France métro`="...22") %>% rename("Code fédération"="...1","Fédération"="Codes régions") %>% slice(91) ) %>%
...
fedsexe[116,1] <- "total"
lic_reg %>%  dplyr::select(1:2,"11":"94",`France métro`="...22") %>% rename("Code fédération"="...1","Fédération"="Codes régions") %>%
    slice(93:115) %>% arrange (desc(`France métro`)) %>% 
  bind_rows(lic_reg %>% dplyr::select(1:2,"11":"94",`France métro`="...22") %>% rename("Code fédération"="...1","Fédération"="Codes régions") %>% slice(116) ) %>%
...
```{r}
  hchart(fede_sexe_dep %>% filter(DEP=="BFC")%>%
           filter(txfem <= 20 | txfem >= 80 ) %>%
          arrange(desc(txfem)) %>%
           left_join(lic_dep %>% select(fede=1,"Fédération"=2),by="fede"),
      "bar", 
  ...
  hc_add_series(fedsexe %>% slice(-1,-39,-40,-91,-92,-116:-124) %>%
           filter(`% de femmes (année 2021)` <= 0.2 | `% de femmes (année 2021)` >= 0.8 ) %>%
          arrange(desc(`% de femmes (année 2021)`)) ,
         "bar", 
         hcaes(x=reorder(`Fédérations françaises agréées en 2021`,`% de femmes (année 2021)`),
  ...
  #bind_rows(summarise_all(., ~if(is.numeric(.)) sum(.) else "Total"))    fonctionne également
   bind_cols( evodep %>%filter(type=="total")  %>% #modifier pour les différents types de fédé
    ...
    left_join(.,feddepsexe %>% select (1,txfem) %>% slice(3:11) %>%                 #txfem
          ...
          (digits(txfem,1,decimal.mark=","))) ,
            by=c("dep"="rowname") ) %>%
    relocate(dep, .before = département) %>% relocate(`Licences féminines (%)`,.before = `évolution 2010-2021`) %>%
rename(`unisport olympique`="Total fédérations unisport olympiques",
       `unisport non olympique`="Total fédérations unisport non olympiques", 
       multisports=`Total fédérations multisports`,
       `Taux de licences (%)`="...6") %>% select(-10) %>%
...
lic_dep %>%  dplyr::select(1:2,depbfc) %>% rename("Code fédération"="...1","Fédération"="Codes départements") %>%
  ...
  mutate(BFC=rowSums(.[3:10],na.rm = T)) %>% arrange (desc(BFC)) %>% adorn_totals("row") %>%
   ...
   left_join(.,fede_sexe_dep %>% select (fede,DEP,txfem) %>% filter (DEP=="BFC") %>%
                         mutate(`Licences féminines (%)`=color_bar('pink',na.rm=T)
                                         (digits(as.numeric(txfem),1,decimal.mark=","))) ,
            by=c("Code fédération"="fede") ) %>% select(-txfem) %>%
...
lic_epci <- sp::merge(epciwgs,lic27epci %>%
  select(CODE_EPCI=EPCI,LIBGEO,licences,txlic,txfemmes,txlicf,txlich,txjeunes,txlicjeunes) %>%
...
```{r,echo=F}
lic27bv %>%
  select(BV2012,LIBGEO,licences,txlic,txlicf,txlich,txlicjeunes,txfemmes,txjeunes) %>% 
  arrange(desc(licences)) %>%
  mutate(txlic = color_bar('orange')
        (digits(txlic,1, decimal.mark=","))) %>%
...
lic_bv <- sp::merge(bvwgs,lic27bv %>%
  select(BV2012,LIBGEO,licences,txlic,txfemmes,txlicf,txlich,txjeunes,txlicjeunes) %>%
    mutate_at(3:8,as.numeric) ,
  by="BV2012")
