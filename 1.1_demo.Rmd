...
region_tab %>%  dplyr::select(Région,Nom,Population,Densité,rang,`Taux femmes`,partZRR,partQPV) %>%
  ...
  footnote(general = " Insee RP2019", general_title = "Source : ",
           number = c("Zone de Revitalisation Rurale 2018 \n","Population des quartiers politique de la ville (QPV) 2018/population RP 2019"), footnote_as_chunk = T) 
...
addPolygons(data=ZRR, group  = "ZRR",fill = T,fillOpacity = 0.4,fillColor = "#6B8E23", stroke=F,weight = 1, smoothFactor = 2) %>%  
  ...
  addCircles(centroid(regwgs)[,1],centroid(regwgs)[,2],
             ...
             popup = popupTable(regwgs@data %>% 
                                   ...
                                   left_join(.,region_tab %>% dplyr::select(Région,Nom,Population,Densité,rang,`Taux femmes`,partZRR,partQPV)
                                             ,by="Région"),
...
dep27_tab %>%  dplyr::select(Région,Département,Nom,Population,Densité,rang,rang_national,`Taux femmes`,partZRR,partQPV) %>%
...
leaflet(depwgs) %>% addProviderTiles(providers$CartoDB.Positron) %>%
  setView(lng = 5.1, lat = 47.27, zoom = 6) %>%
  addPolygons(weight=2,opacity = 0.6,color = "#2F4F4F", fill=F, smoothFactor = 2 ) %>%
  addPolygons(data=densitewgs,fillColor = ~pal(Typdgrd), stroke = F,
              fillOpacity = 0.5, group = "densite" ) %>%  
addPolygons(data=reg27carto,group  = "BFC",fill = F,color="#9370DB",weight = 5,opacity = 1, smoothFactor = 5) %>%
  addPolygons(data=dep27carto,group  = "départements",fill = F,color="#9370DB",weight = 3,opacity = 1, smoothFactor = 5) %>%
  addCircles(centroid(depwgs)[,1],centroid(depwgs)[,2],group = "BV",
               ...
               popup = popupTable(depwgs@data %>% 
                                   ...
                                   left_join(.,dep_tab %>% dplyr::select(Région,Département,Nom,Population,Densité,rang,`Taux femmes`,partZRR,partQPV)
                                             ,by="Département"),
                                 feature.id=F,row.numbers = F)) %>%
  addLegend(pal = pal, values = unique(densitewgs$Lblldgr), opacity = 0.7, title = "grille densité 7 niveaux", position = "bottomright") 
...
```{r,echo=F}
epci27_tab %>%  dplyr::select(EPCI,Nom,Population,Densité,rang,`Taux femmes`,partZRR,partQPV) %>%
...
leaflet(epcicarto) %>% addProviderTiles(providers$CartoDB.Positron) %>%
  ...
  addPolygons(data=densiteBFC,fillColor = ~pal(Typdgrd), stroke = F,
              fillOpacity = 0.5, group = "densite", ) %>%  
  ...
  addCircles(centroid(epcicarto)[,1],centroid(epcicarto)[,2],group = "BV",
               ...
               popup = popupTable(epcicarto@data %>% 
                                   ...
                                   left_join(.,epci27_tab %>% dplyr::select(EPCI,Nom,Population,Densité,rang,`Taux femmes`,partZRR,partQPV)
                                             ,by="EPCI"),
                                 feature.id=F,row.numbers = F)) %>% 
  ...
  addLegend(pal = pal, values = unique(densiteBFC$Lblldgr), opacity = 0.7, title = "grille densité 7 niveaux", position = "bottomright")    
...
```{r,echo=F}
bv27_tab %>%  dplyr::select(BV2012,Nom,Population,Densité,rang,`Taux femmes`,partZRR,partQPV) %>%
...
leaflet(bvcarto) %>% addProviderTiles(providers$CartoDB.Positron) %>%
  ...
  addCircles(centroid(bvcarto)[,1],centroid(bvcarto)[,2],group = "BV",weight=4,
               ...
               popup = popupTable(bvcarto@data %>% 
                                   dplyr::select(BV2012) %>%
                                   left_join(.,bv27_tab %>% dplyr::select(BV2012,Nom,Population,Densité,rang,`Taux femmes`,partZRR,partQPV)
                                             ,by="BV2012"),
                                 feature.id=F,row.numbers = F)) %>%  
  addPolygons(data=QPV27, group  = "QPV",fill = T,color="#2F4F4F",weight = 2,
              highlight = highlightOptions (fillOpacity =  0.4,
...
```{r}
basecom %>% filter(REG=='27') %>% select(CODGEO:SUPERF) %>%
  kable("html", format.args = list(decimal.mark = ",", big.mark = " "),
        align="clrrrrrrrrrrrrrrrrrr",
        escape = F) %>% 
  kable_styling("hover", full_width = F) %>%
