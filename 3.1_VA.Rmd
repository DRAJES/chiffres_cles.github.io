---
title: "Vie associative"
output: 
  flexdashboard::flex_dashboard:
    vertical_layout: scroll
    social: menu
    #source_code: embed
    navbar:
      - { title: "Accueil", href: DRAJESBFC.html, align: right, icon: fa-home }
    theme: simplex
---

```{r setup, include=FALSE ,echo=FALSE, cache=F}

load("data/engagement/asso.RData")
source("R/data/asso.R")

pal2 <- colorBin("YlOrBr", domain =0:10 ,bins= c(0, 3, 3.5, 4, 4.5, 5,5.5,6,10) )

pallic <- colorBin(c("#d7191c","#fec981","#c4e687","#1a9641"), domain =0:100 ,bins= c(0, 19, 22, 25, 100) )


appartenance <- rbind(appartenance,c("REG","FR","France","-"))

RNApop <- RNAcom %>%
      left_join(.,basecom %>% select(CODGEO,LIBGEO,EPCI,BV2012),by=c("CODGEO_2022"="CODGEO"))

coul <- c("#4876FF", "#008B00", "#32CD32","#CD69C9", "#CD96CD", "#FFB5C5" , "#00BFFF","#FF7F50","#ADFF2F","#C0C0C0","#DCDCDC")


# Option de chunk
knitr::opts_chunk$set(
 fig.width = 6,
 fig.asp = 0.8,
 out.width = "80%"
)
```

![](images/BFC_s.svg) Régions {data-orientation="rows" height=30}
=======================================================================
Row {data-height="20"}
-----------------------------------------------------------------------
Row {data-height="100"}
-----------------------------------------------------------------------

### Données générales

En novembre 2022, la Bourgogne-Franche-Comté compte **118 000** associations selon le Répertoire national des Associations.
C'est la troisième région de France en nombre d'association par habitant derrière la Corse et l'Occitanie.
Les associations sportives représentent près du quart des associations (plus grande proportion de France).

Row {.tabset .tabset-fade .tabset-pills data-height="600"}
-----------------------------------------------------------------------

### Comparaisons régionales

**9e** région française en nombre d'associations

```{r}
hchart(RNApop %>% 
         filter (!is.na(REG) & !is.na(objr)) %>% 
         group_by(REG) %>% 
         summarise(asso=sum(total,na.rm = T)) %>%
         left_join(.,pop_basecom(),
                   by="REG") %>%
         arrange(desc(asso)) ,
  "bar", hcaes(x=reorder(Libellé,asso),y=asso, 
               color=ifelse(REG=="27","#6a5acd","#ffd700") ),
         name="nombre d'associations",showInlegend=T, pointWidth = 20 ) %>%
  hc_xAxis(title=list(text="Régions")) %>%
  hc_yAxis(title=list(text="Nombre d'associations")) %>%
  hc_caption(text = "<b>ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA</b>"  ) %>%
  hc_add_theme(thm) 
```
### Taux d'associations

**3e** région française en taux d'associations 

```{r}
hchart(RNApop %>% 
         filter (!is.na(REG) & !is.na(objr)) %>% 
         group_by(REG) %>% 
         summarise(asso=sum(total,na.rm = T)) %>%
         left_join(.,pop_basecom(REG),
                   by="REG") %>%
         adorn_totals(name = "FR", fill = "France") %>%
         mutate(txasso=round(asso*100/pop,2 )) %>%
         arrange(desc(txasso)),
  "bar", hcaes(x=reorder(Libellé,txasso),y=txasso,
               color=ifelse(REG=="27","#6a5acd",ifelse(REG=="FR","#ff7f50","#ffd700") ) ) ,
         name="Taux d'association pour 100 habitants", showInlegend=T, pointWidth = 20  )%>%
  hc_xAxis(title=list(text="Régions")) %>%
  hc_yAxis(title=list(text="Taux d'associations")) %>%
  hc_caption(text = "<b>ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA</b>"  ) %>%
  hc_add_theme(thm) 
```



```{r}
hchart(RNA %>% 
         filter (!is.na(REG) & !is.na(objr)) %>% 
         filter(année > 2010 ) %>%
         group_by(année) %>% count() ,
       "spline",hcaes(x=année,y=n) ) %>%
    hc_caption(text = "<b>ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA</b>"  ) 

```



### Evolution
Créations d'entreprises par année
```{r}
hchart(RNA %>% 
         filter (!is.na(REG) & !is.na(objr)) %>% 
         filter(année > 2010 ) %>%
         group_by(REG, année) %>% count() ,
       "spline",hcaes(x=année,y=n,group=REG) ) %>%
    hc_caption(text = "<b>ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA</b>"  ) 

```




### Carte régionale
Hors Alsace-Moselle
```{r}

RNAcarto <- merge(regwgs,RNAcarto(REG),by="REG")

carte_asso() %>% contour_bfc()
```

Row {.tabset .tabset-fade .tabset-pills data-height="600"}
-----------------------------------------------------------------------

### Tableau 

**Les domaines du sport, des loisirs et de l'économie fortement représentés**

```{r}
RNApop %>% 
  filter (!is.na(REG) & !is.na(objr)) %>% 
  group_by(REG,objr) %>% 
  summarise(asso=sum(total,na.rm = T)) %>%
  pivot_wider(REG,names_from = objr , values_from = asso) %>%
  adorn_totals("col", name = "total" ) %>%
  left_join(.,pop_basecom(REG),
            by="REG") %>%
  adorn_totals("row", name = "FR" ,fill = "France") %>%
  mutate(txasso=total*100/pop) %>%
  
  tableau_asso() %>%
  
  cc_kable("crlrrrrrr") %>%
  column_spec(2, bold=T) %>%
  column_spec(3, italic=T) %>%
    footnote(general = "ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA"  ) %>%
footnote(general = "Répertoire National des Associations (RNA 2022); Insee, RP2019", 
           general_title = "Source : ",
           footnote_as_chunk = T) %>%
  scroll_box(height = "400px") 
```

### Domaines d'intervention des associations


```{r}
hchart(RNApop %>% 
         filter (!is.na(REG) & !is.na(objr)) %>% 
         group_by(REG,objr) %>% 
         summarise(asso=sum(total,na.rm = T)) %>%
         left_join(.,pop_basecom(REG),
                   by="REG") %>%
         #adorn_totals(name = "FR")%>%
         mutate(txasso=round(asso*100/pop,2)),
  "bar", hcaes(x=Libellé,y=asso,group=objr),
         stacking = "normal",
         showInlegend=T, pointWidth = 20)  %>%
    hc_caption(text = "<b>ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA</b>"  ) %>%
  hc_add_theme(thm) 
```


### Structure régionale des domaines d'intervention des associations


```{r}
hchart(RNApop %>% 
         filter (!is.na(REG) & !is.na(objr)) %>%
         group_by(REG,objr) %>% 
         summarise(asso=sum(total,na.rm = T)) %>%
         left_join(.,pop_basecom(REG),
                   by="REG") %>%
         #adorn_totals(name = "FR")%>%
         mutate(txasso=round(asso*100/sum(asso),2)),
  "bar", hcaes(x=Libellé,y=txasso,group=objr),
         stacking = "percent",
         showInlegend=T, pointWidth = 20)  %>%
    hc_caption(text = "<b>ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA</b>"  ) %>%
  hc_add_theme(thm) 
```



![](images/BFC_dep_s.svg) Départements {data-orientation="rows" height=30}
=======================================================================
Row {data-height="20"}
-----------------------------------------------------------------------
Row {.tabset .tabset-fade .tabset-pills data-height="100"}
-----------------------------------------------------------------------


### Données générales

Jura, Nièvre et Haute-Saône : les départements les moins peuplés sont ceux où les associations sont les plus fréquentes

Row {.tabset .tabset-fade .tabset-pills data-height="600"}
-----------------------------------------------------------------------

### Comparaisons départementales

```{r}
hchart(RNApop %>% 
         filter (!is.na(REG) & !is.na(objr)) %>% 
         group_by(REG,DEP) %>% 
         summarise(asso=sum(total,na.rm = T)) %>%
         left_join(.,appartenance %>% 
                     filter(NIVGEO=="DEP") %>%
                     select(CODGEO,département=LIBGEO),
                   by=c("DEP"="CODGEO") )%>%
         arrange(desc(asso)) ,  
  "bar", hcaes(x=reorder(département,asso),y=asso, 
               color=ifelse(REG=="27","#6a5acd","#ffd700") ),
          name="Nombre d'associations",
          showInLegend=T, pointWidth = 1 ) %>%
  
  hc_add_series(RNApop %>% 
                  filter (REG =="27" & !is.na(objr)) %>%
                  group_by(REG,DEP) %>% 
                  summarise(asso=sum(total,na.rm = T)) %>%
                  left_join(.,appartenance %>% 
                              filter(NIVGEO=="DEP") %>%
                              select(CODGEO,département=LIBGEO),
                            by=c("DEP"="CODGEO") )%>%
                  arrange(desc(asso)),  
          "bar",  hcaes(x=reorder(département,asso),y=asso),
                  color="#6a5acd",
                  name="Nombre d'associations",
                  showInLegend=F, pointWidth = 3 ) %>%
    hc_legend(title=list(text=" France/BFC (Clic pour zoomer) " )) %>%
    hc_xAxis(title=list(text="Départements")) %>%
    hc_yAxis(title=list(text="Nombre d'associations")) %>%
    hc_caption(text = "<b>ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA</b>"  ) %>%
    hc_add_theme(thm) 
```

### Taux d'associations

```{r}
hchart(RNApop %>% 
         filter (!is.na(REG) & !is.na(objr)) %>%
         group_by(DEP) %>% 
         summarise(asso=sum(total,na.rm = T)) %>%
         left_join(.,pop_basecom(DEP), 
                   by="DEP") %>%
         adorn_totals(name = "FR", fill = "France") %>%
         
         bind_rows( RNApop %>% 
                      filter (REG=="27" & !is.na(objr)) %>% 
                      group_by(REG) %>%  
                      summarise(asso=sum(total,na.rm = T)) %>%
                      left_join(.,basecom %>%
                                  group_by(REG) %>% 
                                  summarise(pop=sum(pop,na.rm=T)),
                                by="REG") %>%
                      rename(DEP=REG) %>% 
                      mutate(Libellé="Bourgogne-Franche-Comté" )  ) %>%
         mutate(txasso=round(asso*100/pop,2 )) %>%
         mutate(Libellé=ifelse(DEP=="FR","France",Libellé) ) %>%
         arrange(desc(txasso)),  
  "bar", hcaes(x=reorder(Libellé,txasso),y=txasso,
               color=ifelse(DEP %in% depbfc,"#6a5acd",
                     ifelse(DEP=="FR","#ff7f50","#ffd700"))),
         name="Taux d'association pour 100 habitants", 
         showInLegend=T, pointWidth = 1  ) %>%
 
  hc_add_series(RNApop %>% 
                  filter (REG=="27" & !is.na(objr)) %>% 
                  group_by(DEP) %>%  
                  summarise(asso=sum(total,na.rm = T)) %>%
                  left_join(.,pop_basecom(DEP),
                            by="DEP") %>%
                  adorn_totals(name = "BFC", fill = "Bourgogne-Franche-Comté" )%>%
                  mutate(txasso=round(asso*100/pop,2 )) %>%
                  arrange(desc(txasso)),  
          "bar",  hcaes(x=reorder(Libellé,txasso),y=txasso,
                        color=ifelse(DEP %in% depbfc,"#6a5acd",
                              ifelse(DEP=="BFC","#ff7f50","#ffd700"))),
                  name="Taux d'association pour 100 habitants", 
                  showInLegend=F, pointWidth = 3 ) %>%
   
    hc_legend(title=list(text=" France/BFC (Clic pour zoomer) " )) %>%
    hc_xAxis(title=list(text="Départements")) %>%
    hc_yAxis(title=list(text="Taux d'associations")) %>%
    hc_caption(text = "<b>ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA</b>"  ) %>%
    hc_add_theme(thm) 
```

### Evolution
Créations d'entreprises par année

```{r}
hchart(RNA %>%
         filter (!is.na(DEP) & !is.na(objr)) %>% 
         mutate(année=as.numeric( substr(date_creat,1,4) ) )%>%
         filter(année > 2010 & année <2023) %>%
         filter(DEP %in% depbfc) %>%
         group_by(DEP, année) %>% count(),
       "spline",hcaes(x=année,y=n,group=DEP) )
```

### Carte départementale
Hors Alsace-Moselle
```{r}
source("R/data/asso.R")

RNAcarto <- merge(depwgs,RNAcarto(DEP),by="DEP")

carte_asso() %>% contour_bfc()


```

Row {.tabset .tabset-fade .tabset-pills data-height="600"}
-----------------------------------------------------------------------

### Tableau 

Plus de 20 000 associations dans le Doubs, la Côte d'Or et le Saône et Loire

```{r}
RNApop %>% 
  filter (!is.na(REG) & !is.na(objr)) %>% 
  group_by(REG,DEP, objr) %>% 
  filter (DEP %in% depbfc) %>%  
  summarise(asso=sum(total,na.rm = T)) %>%
  pivot_wider(DEP,names_from = objr , values_from = asso) %>%
  adorn_totals("col", name = "total" ) %>%
  left_join(.,pop_basecom(DEP),
            by="DEP") %>%
  adorn_totals("row", name = "BFC" , fill = "Bourgogne-Franche-Comté") %>%
  mutate(txasso=total*100/pop) %>%
  tableau_asso() %>%
  
  cc_kable("crlrrrrrr") %>%
  column_spec(2, bold=T) %>%
  column_spec(3, italic=T) %>%
  footnote(general = "Répertoire National des Associations (RNA 2022); Insee, RP 2019", 
           general_title = "Source : ",
           footnote_as_chunk = T) %>%
  scroll_box(height = "400px") 

```

### Domaines d'intervention des associations


```{r}
hchart(RNApop %>% 
         filter (!is.na(REG) & !is.na(objr)) %>% 
         group_by(REG,DEP,objr) %>% 
         filter (DEP %in% depbfc) %>% 
         summarise(asso=sum(total,na.rm = T)) %>%
         left_join(.,pop_basecom(DEP),
                   by="DEP") %>%
        #adorn_totals(name = "FR")%>%
         mutate(txasso=round(asso*100/pop,2)),
  "bar", hcaes(x=Libellé,y=asso,group=objr),
         stacking = "normal",
         showInlegend=T, pointWidth = 20)  %>%
  hc_add_theme(thm) 
```


### Structure régionale des domaines d'intervention des associations

De nombreuses associations sportives

```{r}
hchart(RNApop %>% 
         filter (!is.na(REG) & !is.na(objr)) %>% 
         group_by(REG,DEP,objr) %>% 
         filter (DEP %in% depbfc) %>% 
         summarise(asso=sum(total,na.rm = T)) %>%
         left_join(.,pop_basecom(DEP),
                   by="DEP") %>%
         #adorn_totals(name = "FR")%>%
         mutate(txasso=round(asso*100/sum(asso),2)),
  "bar", hcaes(x=Libellé,y=txasso,group=objr),
         stacking = "percent",
         showInlegend=T, pointWidth = 20)  %>%
  hc_add_theme(thm) 
```


![](images/BFC_EPCI_s.svg) EPCI {data-orientation="rows" height=30}
=======================================================================
Row {data-height="20"}
-----------------------------------------------------------------------
Row {.tabset .tabset-fade .tabset-pills data-height="900"}
-----------------------------------------------------------------------


### Tableau des associations 
```{r}
RNApop %>% 
  filter (!is.na(REG) & !is.na(objr)) %>%  
  filter(EPCI %in% basecom$EPCI[basecom$REG=="27"]) %>%
  group_by(EPCI, objr)  %>%  
  summarise(asso=sum(total,na.rm = T)) %>%
  pivot_wider(EPCI,names_from = objr , values_from = asso) %>%
  adorn_totals("col", name = "total" ) %>% 
  left_join(.,pop_basecom(EPCI),
            by="EPCI") %>%
  arrange(desc(total)) %>%
  adorn_totals("row", name = "BFC" ,fill = "Bourgogne-Franche-Comté") %>% 
  mutate(txasso=total*100/pop) %>%
  tableau_asso() %>%
  
  cc_kable("rcrlrrrrr") %>%
  column_spec(3, bold=T) %>%
  column_spec(4, italic=T) %>%
  footnote(general = "Répertoire National des Associations (RNA 2022); Insee, RP 2019", 
           general_title = "Source : ",
           footnote_as_chunk = T) %>%
  scroll_box(height = "800px") 

```

### Carte EPCI

```{r}
source("R/data/asso.R")

RNAcarto <- merge(epcicarto,RNAcarto(EPCI),by="CODE_EPCI")

carte_asso() %>% contour_depbfc()

```

![](images/BFC_BV_s.svg) Bassins de vie {data-orientation="rows" height=30}
=======================================================================
Row {data-height="20"}
-----------------------------------------------------------------------
Row {.tabset .tabset-fade .tabset-pills data-height="900"}
-----------------------------------------------------------------------


### Tableau des associations 
```{r}
RNApop %>% 
  filter (!is.na(REG) & !is.na(objr)) %>%  
  filter(BV2012 %in% basecom$BV2012[basecom$REG=="27"]) %>%
  group_by(BV2012, objr)  %>%  
  summarise(asso=sum(total,na.rm = T)) %>%
  pivot_wider(BV2012,names_from = objr , values_from = asso) %>%
  adorn_totals("col", name = "total" ) %>% 
  left_join(.,pop_basecom(BV2012),
            by="BV2012") %>%
  arrange(desc(total)) %>%
  adorn_totals("row", name = "BFC" ) %>% 
  mutate(txasso=total*100/pop) %>%
  tableau_asso() %>%
  
  cc_kable("rcrlrrrrr") %>%
  column_spec(3, bold=T) %>%
  column_spec(4, italic=T) %>%
  footnote(general = "Répertoire National des Associations (RNA 2022); Insee, RP 2019", 
           general_title = "Source : ",
           footnote_as_chunk = T) %>%
  scroll_box(height = "800px") 

```

### carte BV

```{r}
source("R/data/asso.R")

RNAcarto <- merge(bvcarto,RNAcarto(BV2012),by="BV2012")

carte_asso() %>% contour_depbfc()
```


*En savoir* **+** {data-orientation="rows" data-icon="fa-info-circle" font-size="25px" }
=======================================================================
Row {data-height="20"}
-----------------------------------------------------------------------
Row {data-height="60"}
-----------------------------------------------------------------------

**Sources :**  

**ATTENTION : Les associations d'Alsace-Moselle relèvent du droit local, indépendant de la loi de 1901, et ne sont pas enregistrées dans le RNA.**


* Répertoire National des Associations (RNA)  

  + https://www.data.gouv.fr/fr/datasets/repertoire-national-des-associations/  
  
* Recherche et solidarités  

  + https://recherches-solidarites.org/dans-les-regions-liste/#Bourgogne


