...
leaflet(RNAcarto) %>%  addTiles(group = "OSM",options = providerTileOptions(opacity = 0.6)) %>%
 addProviderTiles(providers$CartoDB.Positron) %>%
  ...
  addPolygons(data=RNAcarto,group  = "régions",weight = 2,color = "#2F4F4F", smoothFactor = 2,
...
RNApop %>% filter (!is.na(REG) & !is.na(objr)) %>% 
  ...
  row_spec(13,  bold = T ) %>%
  footnote(general = "Répertoire National des Associations (RNA 2020); Insee, RP2019", general_title = "Source : ",
            footnote_as_chunk = T) %>%
...
RNApop %>% filter (!is.na(REG) & !is.na(objr)) %>% 
  ...
  column_spec(3, italic=T) %>%
  row_spec(9,  bold = T ) %>%
  footnote(general = "Répertoire National des Associations (RNA 2020); Insee, RP 2019", general_title = "Source : ",
            footnote_as_chunk = T) %>%
   scroll_box(height = "400px") 
```
