...
epci_tab <- EPCI %>% 
  ...
  row_spec(which (EPCI %in% basecom$EPCI[basecom$REG=="27"]),background = "#E0FFFF75",  bold = T , color = "orange"  )%>%
  row_spec(1234,bold = T,color = "steelblue") %>%
  row_spec(1235,bold = T,color = "navy") %>%
...
#  save_kable(epci_tab,file = "P:/projets R/DRAJES/tableaux/epci.html", self_contained = T)
#epci_tab
...
epci27_tab <- EPCI %>% filter(EPCI %in% basecom$EPCI[basecom$REG=="27"]) %>%
  ...
  row_spec(117,bold = T,color = "steelblue") %>%
  row_spec(118,bold = T,color = "navy") %>%
  add_header_above(c(" " = 4, "Jeunes" =3, "Pop âgée" = 3, " "= 1, "Jeunes"=2,"Pop âgée"=2," "=2)) %>%
  add_header_above(c(" " = 10, "Evolution 2012-2017" =5, " "= 2)) %>%
  footnote(general = " Insee RP2012, RP2017", general_title = "Source : ", footnote_as_chunk = T) 

 # save_kable(epci27_tab,file = "P:/projets R/DRAJES/tableaux/epci27.html", self_contained = T)
