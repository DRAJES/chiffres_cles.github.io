...
fede_sexe_dep <- fedsexeuni %>% select(1:2,depbfc)%>% slice(2:149,151:154) %>% 
       bind_rows(fedsexenoly %>% select(1:2,depbfc)%>% slice(2:201,203:206)) %>%
       bind_rows(fedsexemulti %>% select(1:2,depbfc)%>% slice(2:93,95:98)) 

fede_sexe_dep  <- fede_sexe_dep %>% select(-1) %>% bind_cols(
                    fede_sexe_dep %>% select(...1) %>% filter(!is.na(...1)) %>%
...
lic27epci <- licences27 %>% group_by(CODGEO,EPCI,pop,popf,poph,p20) %>%
...
lic27bv <- licences27 %>% group_by(CODGEO,BV2012,pop,popf,poph,p20) %>%
