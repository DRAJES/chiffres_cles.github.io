...
nature %>% 
  ...
  left_join(.,appartenance %>% 
              dplyr::filter (NIVGEO=="REG") %>% 
              dplyr::select (2,3),
            by=c("REG"="CODGEO")) %>%
  relocate(région=LIBGEO,.after=REG) %>%
  
 cc_kable("lcrrrrrrrrr") %>%
...
hchart(nature %>% 
         ...
         arrange(desc(`sites de nature`)) %>%
         left_join(.,appartenance %>% 
                     dplyr::filter (NIVGEO=="DEP") %>% 
                     dplyr::select (2,3),
                   by=c("DEP"="CODGEO")) %>%
         relocate(département=LIBGEO,.after=DEP) ,
"bar", hcaes(x=reorder(département,`sites de nature`),
                    y=`sites de nature`,
...
nature27 %>% 
  filter (REG=="27") %>% 
  group_by(EquipementFamille,EquipementTypeLib) %>% 
...
dep27carto <- merge(dep27carto,depnature27,by.x="DEP",by.y="Var1")
