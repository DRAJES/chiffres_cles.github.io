...
```{r}
carte <- regwgs
carte@data <- carte@data %>% select(Région=REG,pop,p1529,ind_jeun)
...
```{r}
carte <- depwgs
carte@data <- carte@data %>% select(Département=DEP,pop,p1529,ind_jeun)
...
```{r}
carte <- epcicarto
carte@data <- carte@data %>% select(EPCI,pop,p1529,ind_jeun)
...
```{r}
carte <- bvcarto
carte@data <- carte@data %>% select(BV2012,pop,p1529,ind_jeun)
