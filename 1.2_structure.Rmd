...
```{r}
carte <- regwgs |> 
  dplyr::select(Région=REG,geometry,pop,p1529,ind_jeun)  |>  
  left_join(tab ,
            by=join_by(Région))
...
```{r}
carte <- depwgs |> 
  dplyr::select(Département=DEP,geometry,pop,p1529,ind_jeun)
...
```{r}
carte <- epcicarto |> 
  dplyr::select(EPCI=CODE_SIREN,geometry,pop,p1529,ind_jeun)  |>  
  left_join(tab,
            by=join_by(EPCI))
...
```{r}
carte <- bvcarto |> 
  dplyr::select(BV2022,geometry,pop,p1529,ind_jeun)  |>  
  left_join(tab,
            by=join_by(BV2022))
