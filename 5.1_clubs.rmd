...
hchart(clubdep %>% slice(-97) %>% arrange(desc(V1)),"bar", hcaes(x=reorder(departement,V1),y=V1,color=ifelse(dep %in% depbfc,"#6a5acd","#ffd700") ), 
       showInLegend = T, name = "Clubs sportifs FR", pointWidth = 2) %>%
  
    hc_add_series(clubdep %>% filter(dep %in% depbfc) %>% arrange(desc(V1)),"bar", hcaes(x=reorder(departement,V1),y=V1,color=ifelse(dep %in% depbfc,"#6a5acd","#ffd700") ), 
       showInLegend = T, name = "Clubs sportifs BFC", pointWidth = 2) %>%
...
hchart(clubdep %>% arrange(desc(txclub)),"bar", hcaes(x=reorder(departement,txclub),y=txclub,color=ifelse(dep %in% depbfc,"#6a5acd",
       ...
       showInLegend = T, name = "France", pointWidth = 1) %>%
  
  hc_add_series(clubdep %>% filter(dep %in% depbfc | dep == "FR" ) %>% arrange(desc(txclub)),"bar", hcaes(x=reorder(departement,txclub),y=txclub,color=ifelse(dep %in% depbfc,"#6a5acd","#ffd700") ), 
       showInLegend = T, name = "BFC", pointWidth = 1) %>%
