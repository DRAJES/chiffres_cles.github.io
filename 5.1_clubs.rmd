...
load("C:/Users/plebre/Documents/projets R/DRAJES/data/sport/licences.RData")
load("C:/Users/plebre/Documents/projets R/DRAJES/data/sport/clubs.RData")
...
load("C:/Users/plebre/Documents/projets R/DRAJES/data/sport/spark.RData")
...
clubdep <- clubdep %>% rownames_to_column(var="dep") %>% mutate (dep=ifelse(nchar(dep)==1,paste0('0',dep),dep), V1=as.numeric(V1)) %>%
                 left_join(.,depwgs@data %>% dplyr::select(dep=INSEE_DEP,pop),by="dep" )    %>%
  adorn_totals("row",name = "FR") %>%
                  mutate(txclub=round(10000*V1/pop,1)) 
clubdep[97,3] <- "France"

hchart(clubdep %>% slice(-97) %>% arrange(desc(V1)),"bar", hcaes(x=reorder(departement,V1),y=V1,color=ifelse(dep %in% depbfc,"#6a5acd","#ffd700") ), 
       showInLegend = F, name = "Clubs sportifs", pointWidth = 2) %>%
  hc_add_theme(thm) 
```
...
hchart(clubdep %>% arrange(desc(txclub)),"bar", hcaes(x=reorder(departement,txclub),y=txclub,color=ifelse(dep %in% depbfc,"#6a5acd",
                                                                 ifelse(dep=="FR","#ff7f50","#ffd700") ) ), 
       showInLegend = F, name = "Clubs sportifs", pointWidth = 1) %>%
  hc_add_theme(thm) 
...
```{r}
hchart(clubdep %>% filter(dep %in% depbfc) %>% arrange(desc(V1)),"bar", hcaes(x=reorder(departement,V1),y=V1,color=ifelse(dep %in% depbfc,"#6a5acd","#ffd700") ), 
       showInLegend = F, name = "Clubs sportifs", pointWidth = 20) %>%
  hc_add_theme(thm) 
```

### Taux infra régionaux pour 10 000 habitants

```{r}

hchart(clubdep %>% filter(dep %in% depbfc) %>% arrange(desc(txclub)),"bar", hcaes(x=reorder(departement,txclub),y=txclub,color=ifelse(dep %in% depbfc,"#6a5acd","#ffd700") ), 
       showInLegend = F, name = "Clubs sportifs", pointWidth = 20) %>%
  hc_add_theme(thm) 
...
leaflet(club_epci) %>%  addProviderTiles(providers$Wikimedia) %>%
...
leaflet(club_bv) %>%  addProviderTiles(providers$Wikimedia) %>%
  setView(lng = 5.1, lat = 47.27, zoom = 8) %>%
  addPolygons(fill =F , weight = 2,opacity = 0.6,
              color = "#2F4F4F",fillOpacity = 0.7 ) %>%
  addPolygons(data=reg27carto,group  = "BFC",fill = F,color="#9370DB",weight = 5,opacity = 1) %>%
