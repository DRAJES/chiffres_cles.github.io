...
hchart(clubreg %>% 
         arrange(desc(txclub)),
"bar",hcaes(x=reorder(region,txclub),y=txclub,
...
leaflet(cartosportreg) %>% 
  ...
  addCircles(centroid(cartosportreg)[,1],centroid(cartosportreg)[,2],
             group = "region", 
             radius=~500*clubs_tot^(1/2),
             fillColor = ~pal2(10000*cartosportreg$clubs_tot/cartosportreg$pop), fillOpacity = 0.8,
  ...
  addLegend( pal = pal2 , values = 10000*(cartosportreg$clubs_tot/cartosportreg$pop),
             position = "bottomright", title = "taux de clubs sportifs" )
...
club_reg %>%  
  ...
  dplyr::rename("Total clubs"="NA",REG=variable) %>%
  left_join( lic_reg %>% 
               dplyr::select(2,"11":"94",France="...22") %>%
               slice(117) %>%
               pivot_longer(-`Codes régions`, 'variable', 'value') %>%
...
club_reg %>%  
  ...
  kable("html", format.args = list(decimal.mark = ",", big.mark = " "),
      align="clrrrrrrrrrrrrrrrl", escape = F) %>% 
  kable_styling("hover", full_width = F) %>%
...
leaflet(cartosportdep) %>% 
  ...
  addCircles(centroid(cartosportdep)[,1],centroid(cartosportdep)[,2],
             ...
             popup = popupTable(cartosportdep@data %>% 
                                  ...
                                  mutate(`Taux de clubs`=10000*clubs_tot/pop),
                                feature.id=F,row.numbers = F)) %>%
  addLegend( pal = pal2 , values = 10000*(cartosportdep$clubs_tot/cartosportdep$pop),
             position = "bottomright", title = "taux de clubs sportifs" )
