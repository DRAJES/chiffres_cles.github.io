...
```{r}
clubreg <- as.data.frame(t(club_reg[119,3:15]))
...
hchart(clubreg %>% slice(-14) %>% arrange(desc(V1)),"bar",
...
hchart(clubreg %>% arrange(desc(txclub)),"bar",
       hcaes(x=reorder(region,txclub),y=txclub,
             color=ifelse(REG=="27","#6a5acd",
                   ifelse(REG=="FR","#ff7f50","#ffd700") ) ) , 
       showInLegend = F, name = "Taux de clubs", pointWidth = 10) %>%
    hc_xAxis(title=list(text="Région")) %>%
    hc_yAxis(title=list(text="Taux de clubs (/10 000)")) %>%
    hc_add_theme(thm) 
...
sportreg <- as.data.frame(t(club_reg)) %>%
            select(clubs_tot=V119) %>% 
            mutate(clubs_tot=as.numeric(clubs_tot)) %>%
...
leaflet(cartosportreg) %>% 
  ...
  addCircles(centroid(cartosportreg)[,1],centroid(cartosportreg)[,2],
             ...
             popup = popupTable(cartosportreg@data %>% 
                                   select(Région=LIBGEO,pop,clubs_tot) %>%
                                  mutate(`Taux de clubs`=10000*clubs_tot/pop),
                                 feature.id=F,row.numbers = F)) %>%
addLegend( position = "bottomright",pal = pal2 ,title = "taux de clubs sportifs",
           values = 10000*(cartosportreg$clubs_tot/cartosportreg$pop) )
```
...
hchart( club_reg %>%  dplyr::select(2,"11":"94"
    ...
    pivot_longer(-`Codes régions`, 'REG') %>% 
      rename(type_fede=`Codes régions`,clubs=value) %>%
...
club_reg %>%  
  dplyr::select(1:2,"11":"94",France="...22") %>% 
  rename("Code fédération"="...1","Fédération"="Codes régions") %>%
  slice(2:38) %>% 
  arrange (desc(France)) %>% 
  bind_rows(club_reg %>% 
              dplyr::select(1:2,"11":"94",France="...22") %>% 
              rename("Code fédération"="...1","Fédération"="Codes régions") %>% slice(39) ) %>%
  left_join(lic_reg %>% 
...
hchart(clubdep %>% 
  ...
  "bar", hcaes(x=reorder(departement,V1),y=V1,
          ...
          showInLegend = T, name = "Nombre de clubs", pointWidth = 1) %>%
  hc_add_series(clubdep %>% 
                  filter(dep %in% depbfc) %>% 
                  arrange(desc(V1)),
           "bar", hcaes(x=reorder(departement,V1),y=V1) ,
                  color="#6a5acd" ,
                  showInLegend = F, name = "Nombre de clubs", pointWidth = 3) %>%
...
hchart(clubdep %>% 
  ...
  "bar", hcaes(x=reorder(departement,txclub),y=txclub,
          ...
          showInLegend = T, name = "Taux de clubs / 10 000", pointWidth = 1) %>%
  hc_add_series(clubdep %>% 
...
leaflet(cartosportdep) %>% 
  ...
  addCircles(centroid(cartosportdep)[,1],centroid(cartosportdep)[,2],
             ...
             popup = popupTable(cartosportdep@data %>% 
                                ...
                                feature.id=F,row.numbers = F)) %>%
  addLegend( position = "bottomright",pal = pal2 ,title = "taux de clubs sportifs",
             values = 10000*(cartosportdep$clubs_tot/cartosportdep$pop) )
```
...
```{r,echo=F}
lic27epci %>%
  select(EPCI,LIBGEO,`nombre de clubs`=total_clubs,`nombre de licences` = licences, txclub, txlic, liclub)  %>% 
...
club_epci <- sp::merge(epciwgs,lic27epci %>%
                         select(CODE_EPCI=EPCI,LIBGEO,total_clubs,licences,
                                txclub,txlic,liclub) %>%
  ...
  addCircles(centroid(club_epci)[,1],centroid(club_epci)[,2],weight=4,
             ...
             popup = popupTable(club_epci@data %>% 
                                  ...
                                  mutate(liclub = color_tile('white','lightgreen',alpha=0.5)
                                        (digits(liclub ,1,decimal.mark=","))),
                                feature.id=F,row.numbers = F) ) %>%
  addLegend( position = "bottomright",pal = pal2 ,values = ~txclub,
             title = "taux de clubs")
  ...
  addCircles(centroid(club_epci)[,1],centroid(club_epci)[,2],weight=4,
             ...
             popup = popupTable(club_epci@data %>% 
                                  ...
                                  mutate(liclub = color_tile('white','lightgreen',alpha=0.5)
                                        (digits(liclub ,1,decimal.mark=","))),
                                feature.id=F,row.numbers = F) ) %>%
addLegend( position = "bottomright",pal = pallic ,values = ~txclub,
