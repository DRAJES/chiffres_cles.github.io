...
Des missions tournées principalement vers les actions humanitaires et la solidarité.
...
cartedep <- basebfcsig %>%
  ...
  left_join(.,appartenance %>% 
              ...
              select(CODGEO,département=LIBGEO),
            by=c("DEP"="CODGEO") ) %>%
  rename(`Volontaires 2020`=tot20,`Taux de volontaires parmi les 16-25 ans`=txvol)
...
leaflet(cartedep) %>% 
  ...
  addCircles(centroid(cartedep)[,1],centroid(cartedep)[,2], group = "region",
             ...
             popup = popupTable(cartedep@data %>%
                                  select(DEP,département,`Volontaires depuis création`=tot,
