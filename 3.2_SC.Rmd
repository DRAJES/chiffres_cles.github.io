...
leaflet() %>%
 addProviderTiles(providers$CartoDB.Positron) %>% 
  setView(lng = 5.1, lat = 47.27, zoom = 8) %>%
  addPolygons(data=dep27carto, weight=2,opacity = 0.6,color = "#2F4F4F", fill=F, smoothFactor = 2 ) %>%
  addPolygons(data=reg27carto,group  = "BFC",fill = F,color="#9370DB",weight = 5, opacity = 1, smoothFactor = 5) %>%
...
basebfcsig %>%filter(depvol %in% depbfc) %>%
  ...
  column_spec(3, bold=T) %>%
  row_spec(9,background = "#E0FFFF75",  bold = T , color = "steelblue") %>%
  footnote(general = "Agence du Service Civique, Elisa; Insee, RP 2019. Traitements DRAJES", general_title = "Source : ",
            footnote_as_chunk = T)
