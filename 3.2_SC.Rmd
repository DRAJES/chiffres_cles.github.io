...
load("C:/Users/plebre/Documents/projets R/DRAJES/data/engagement/SC.RData")
...
cartedep <- basebfcsig %>%filter(depvol %in% depbfc) %>%
  ...
  left_join(.,basecomQPV %>% filter(DEP %in% depbfc) %>% group_by(DEP) %>% summarise(pop=sum(pop),popQPV=sum(POP_MUN),pop1625=sum(p1625)) %>% adorn_totals(name="BFC"),by="DEP") %>%
