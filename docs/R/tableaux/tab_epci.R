...
epci27_tab <- EPCI %>% filter(EPCI %in% basecom$EPCI[basecom$REG=="27"]) %>%
  ...
  mutate( densite =  color_tile('white','grey')
          ...
          partZRR=ifelse(partZRR>0, color_bar('lightgreen')
                         (comma(partZRR,digits=1,big.mark = " ",decimal.mark=",")),"0"),
          partQPV=ifelse(partQPV>0, color_bar('orange')
                         (comma(partQPV,digits=1,big.mark = " ",decimal.mark=",")),"0")
  )    %>% slice(1:116) %>%
